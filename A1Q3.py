...
# Constants for the 24-puzzle
PUZZLE_SIZE = 5  # 5x5 grid
...
def get_tile_position(state, tile):
    ...
    for i in range(PUZZLE_SIZE):
        for j in range(PUZZLE_SIZE):
            if state[i][j] == tile:
                return (i, j)
    # Raise an error if the tile is not found
    raise ValueError(f"Tile {tile} not found in the puzzle state. State: {state}")
...
def generate_random_puzzle(moves=50):
    ...
    empty_pos = (4, 4)  # Start from the solved position
...
def a_star_solver(start_state, heuristic_func):
    """A* search algorithm to solve the 24-puzzle."""
    open_list = []
...
# Generate random puzzles and solve them using h1, h2, and h3
random_puzzles = [generate_random_puzzle(moves=50) for _ in range(100)]
results = []
...
print("A* Algorithm Results for 24-Puzzle")
